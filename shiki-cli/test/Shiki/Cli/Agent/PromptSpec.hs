...
tests =
  testGroup
    ...
    [ testCase "renders services, runs, and structural headings" $ do
        ...
        assertContains "service-bar line"
        ...
        assertBool "no curly placeholders left"
    ...
    , testCase "empty services and runs render empty placeholders" $ do
        let rendered = renderAssistPrompt
...
assertContains label needle haystack =
  assertBool (label <> ": expected to find " <> show needle)
