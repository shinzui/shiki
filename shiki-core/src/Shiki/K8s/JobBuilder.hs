...
buildJob svc snap inputs =
  ...
  where
    ...
    spec = (K8s.mkV1JobSpec podTemplate)
    ...
    templateMetadata = K8s.mkV1ObjectMeta
    ...
    mainContainer = (K8s.mkV1Container (svc ^. #containerName))
...
mkInitContainer snap ic =
  (K8s.mkV1Container (ic ^. #name))
    { K8s.v1ContainerImage         = Just (ic ^. #image)
...
toV1EnvVar snap ev = case ev ^. #source of
  ConfigMap k ->
    (K8s.mkV1EnvVar (ev ^. #name))
      { K8s.v1EnvVarValueFrom = Just K8s.mkV1EnvVarSource
          { K8s.v1EnvVarSourceConfigMapKeyRef = Just (K8s.mkV1ConfigMapKeySelector k)
  ...
  Secret k ->
    (K8s.mkV1EnvVar (ev ^. #name))
      { K8s.v1EnvVarValueFrom = Just K8s.mkV1EnvVarSource
          { K8s.v1EnvVarSourceSecretKeyRef = Just (K8s.mkV1SecretKeySelector k)
...
toResourceRequirements r = K8s.mkV1ResourceRequirements
  { K8s.v1ResourceRequirementsRequests = Just (Map.fromList
      [ ("cpu",    Quantity (r ^. #cpuRequest))
      , ("memory", Quantity (r ^. #memoryRequest))
      ])
  , K8s.v1ResourceRequirementsLimits   = Just (Map.fromList
